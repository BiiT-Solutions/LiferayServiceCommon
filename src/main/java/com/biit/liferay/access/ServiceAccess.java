...
public abstract class ServiceAccess<Type, LiferayType extends Type> implements LiferayService, IServiceAccess {
    ...
    @Override
    public void authorizedServerConnection(String host, String protocol, int port, String proxyPrefix, String webservicesPath,
                                           String authenticationToken, String loginUser, String password) {
        this.host = host;
        this.protocol = protocol;
        this.port = port;
        this.proxyPrefix = parseProxyPrefix(proxyPrefix);

        LiferayClientLogger.debug(this.getClass().getName(),
                "Accessing using protocol '" + protocol + "', host '" + host + "', port '" + port + "', path '"
    ...
    private String getHttpGetResponse(String webservicesPath, String webService, List<NameValuePair> params,
        ...
        if (useAuthorization) {
            LiferayClientLogger.debug(ServiceAccess.class.getName(),
                    "Accessing with credentials to '" + targetHost + "' service '" + get + "'!");
            response = getAuthorizedHttpClient().execute(targetHost, get, httpContext);
        } else {
            LiferayClientLogger.debug(ServiceAccess.class.getName(), "Accessing without credentials!");
            response = getHttpClientWithoutCredentials().execute(targetHost, get, httpContext);
        }
        if (response.getEntity() != null) {
            // A Simple JSON Response Read
            String result = EntityUtils.toString(response.getEntity());

            if (result.contains(NOT_AUTHORIZED_ERROR_LIFERAY_621)
                    || result.contains(NOT_AUTHORIZED_ERROR_LIFERAY_625)) {
                if (!useAuthorization) {
                    LiferayClientLogger.debug(ServiceAccess.class.getName(),
                            "Accessing to '" + webService + "' without authorization. Retry with authorization ("
            ...
            LiferayClientLogger.debug(ServiceAccess.class.getName(),
                    "Accessing to '" + webService + "' (" + (System.currentTimeMillis() - startTime) + " ms).");
    ...
    private String getHttpPostResponse(String webservicesPath, String webService, List<NameValuePair> params,
        ...
        if (useAuthorization) {
            LiferayClientLogger.debug(ServiceAccess.class.getName(),
                    "Accessing with credentials to '" + targetHost + "' service '" + post + "'!");
     ...
     */
    public String getHttpPostResponse(String webService, MultipartEntityBuilder builder)
            throws ClientProtocolException, IOException, NotConnectedToWebServiceException, AuthenticationRequired {
        return getHttpPostResponse(webService, builder, true);
    }
     ...
     * @throws AuthenticationRequired
     */
    public String getHttpPostResponse(String webService, MultipartEntityBuilder builder, boolean useAuthorization)
            throws ClientProtocolException, IOException, NotConnectedToWebServiceException, AuthenticationRequired {
        long startTime = System.currentTimeMillis();
        ...
        if (useAuthorization) {
            LiferayClientLogger.debug(ServiceAccess.class.getName(),
                    "Accessing with credentials to '" + targetHost + "' service '" + httpPost + "'!");
    ...
    protected String parseProxyPrefix(String proxyPrefix) {
        // Add URL separators.
        if (proxyPrefix == null) {
            proxyPrefix = "";
        }
        return proxyPrefix + (proxyPrefix.endsWith("/") || proxyPrefix.length() == 0 ? "" : "/");
    }
    ...
    public void serverConnection() {
        ...
        if (isNotConnected()) {
            ...
            String proxyPrefix = LiferayConfigurationReader.getInstance().getProxyPrefix();
            String authenticationToken = LiferayConfigurationReader.getInstance().getAuthToken();

            authorizedServerConnection(host, protocol, port, proxyPrefix, webservicesPath, authenticationToken, loginUser, password);
        }
    }
    ...
    public void serverConnection(String user, String password) {
        ...
        authorizedServerConnection(host, protocol, port, proxyPrefix, webservicesPath, authenticationToken, user, password);
    }
