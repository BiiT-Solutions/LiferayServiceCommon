...
public class OrganizationService extends ServiceAccess<IGroup<Long>, Organization> implements IOrganizationService {
	...
	public IGroup<Long> addOrganization(IGroup<Long> company, Long parentOrganizationId, String name, OrganizationType type, Long regionId, Long countryId,
			int statusId, String comments, boolean site) throws NotConnectedToWebServiceException, ClientProtocolException, IOException, AuthenticationRequired,
	...
	public void addUsersToOrganization(List<IUser<Long>> users, IGroup<Long> organization)
		...
		if (users != null && organization != null && !users.isEmpty()) {
			...
			for (int i = 0; i < users.size(); i++) {
				usersIds += users.get(i).getUniqueId();
				if (i < users.size() - 1) {
					usersIds += ",";
				}
			}
	...
	public boolean deleteOrganization(IGroup<Long> company, IGroup<Long> organization)
			throws NotConnectedToWebServiceException, ClientProtocolException, IOException, AuthenticationRequired, OrganizationNotDeletedException {
	...
	public IGroup<Long> getOrganization(Long organizationId)
			throws JsonParseException, JsonMappingException, IOException, NotConnectedToWebServiceException, WebServiceAccessError, AuthenticationRequired {
	...
	public Set<IGroup<Long>> getOrganizations(IGroup<Long> site, IUser<Long> user)
			throws NotConnectedToWebServiceException, ClientProtocolException, IOException, AuthenticationRequired, PortletNotInstalledException {
		Set<IGroup<Long>> organizations = new HashSet<IGroup<Long>>();
	...
	public Long getOrganizationId(IGroup<Long> company, String name)
			throws NotConnectedToWebServiceException, ClientProtocolException, IOException, AuthenticationRequired, InvalidParsedElement {
